...
%
function DuffingOscillatorCDMDOCRComparison()
...
ScalingFactor = 1;
tic
[~,~,~,~,fc] = ConvergentLiouvilleDMD(Kd,Kr,State,SampleTime,PinvTol=PinvTol);
toc
[~,~,~,~,fr] = ConvergentLiouvilleDMD(Kd,Kr,State,SampleTime,RegTol=RegTol);
...
for i=1:size(IVeval,2)
    ...
    x_dot_hat_at_x0_r = [x_dot_hat_at_x0_r fr(x0)];
...
disp(['Maximum vectorfield estimation error for DMD is ' num2str(max(max(abs(x_dot_at_x0 - x_dot_hat_at_x0_r))))])
% figure
